 ...
 * 
 * - 클라이언트 요청 시 생성되는
 *   HttpServletRequest, HttpServletResoponse가
 ...
 *   
 *   - 여러 필터를 만들어 연쇄적으로 연결하여 수행 가능 (FilterChain)
 * 
 */
...
// filterName 속성 : 필터의 이름 지정, 필터 순서 지정시 사용
...
public class EncodingFilter extends HttpFilter implements Filter {
	...
	// 서버 실행시 또는 필터 코드 변경시 필터가 자동 생성되는데
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// ServletResponse == HttpSerletResponse의 부모 타입
		// -> 필요시 다운 캐스팅
